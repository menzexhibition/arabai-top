...
function header(active = "models") {
  return `<header class="platform-header"><a class="platform-brand" href="../index.html" aria-label="الصفحة الرئيسية ARABAI"><img src="../assets/brand/arabai-logo-320.png" alt="" width="44" height="50" /><span>ARABAI</span></a><nav class="platform-nav" aria-label="التنقل الرئيسي"><a href="../index.html">الرئيسية</a><a href="../models.html"${active === "models" ? ' aria-current="page"' : ""}>سوق النماذج</a><a href="../console.html">لوحة التحكم</a><a href="../learn.html">دليل AI</a></nav><div class="platform-account-actions"><a class="header-register" href="#early-access">القائمة المبكرة</a></div></header>`;
}
...
function comparisonPage(item) {
  ...
  const tableRows = models.map((model) => `<tr><td><a class="model-name-link" href="../models/${slug(model.model)}.html"><strong dir="ltr">${escapeHtml(model.model)}</strong></a></td><td>${escapeHtml(model.vendor || "-")}</td><td>${categoryCopy(model)[0]}</td><td>${model.unit === "request" ? `<span dir="ltr">${usd(model.arabaiPriceUsd)}</span> لكل طلب` : `<span dir="ltr">${usd(model.arabaiInputUsd)} / ${usd(model.arabaiOutputUsd)}</span> لكل 1M token`}</td></tr>`).join("");
