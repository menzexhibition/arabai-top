...
```bash
# Install all agents to your Claude Code directory./scripts/install.sh --tool claude-code
...
```bash
# Step 1 -- generate integration files for all supported tools./scripts/convert.sh

# Step 2 -- install interactively (auto-detects what you have installed)./scripts/install.sh

# Or target a specific tool directly./scripts/install.sh --tool antigravity./scripts/install.sh --tool gemini-cli./scripts/install.sh --tool opencode./scripts/install.sh --tool copilot./scripts/install.sh --tool openclaw./scripts/install.sh --tool cursor./scripts/install.sh --tool aider./scripts/install.sh --tool windsurf./scripts/install.sh --tool kimi./scripts/install.sh --tool codex
```
...
| 🎬 [Bilibili Content Strategist](marketing/marketing-bilibili-content-strategist.md) | B algorithm, danmaku culture, UP growth | Building audiences on Bilibili with community-first content |
...
| 🎥 [Kuaishou Strategist](marketing/marketing-kuaishou-strategist.md) | Kuaishou, community, grassroots growth | Building authentic audiences in lower-tier markets |
...
| 📡 [Multi-Platform Publisher](marketing/marketing-multi-platform-publisher.md) | One-click Chinese multi-platform publishing | Routing one article to //CSDN/B// |
...
```bash./scripts/install.sh --tool cursor./scripts/install.sh --tool opencode./scripts/install.sh --tool openclaw./scripts/install.sh --tool antigravity./scripts/install.sh --tool codex
```
...
```bash./scripts/install.sh --tool copilot
```
...
```bash./scripts/convert.sh --tool gemini-cli./scripts/install.sh --tool gemini-cli
```
...
```bash./scripts/convert.sh --tool openclaw./scripts/install.sh --tool openclaw
```
...
```bash
# Convert and install./scripts/convert.sh --tool kimi./scripts/install.sh --tool kimi
```
...
```bash./scripts/convert.sh --tool codex./scripts/install.sh --tool codex
```
...
```bash./scripts/convert.sh # regenerate all (serial)./scripts/convert.sh --parallel # regenerate all in parallel (faster)./scripts/convert.sh --tool codex # regenerate just one tool./scripts/convert.sh --tool cursor # regenerate just one tool
```
...
|----------|-----------|------|-------|
| 🇨🇳 (zh-CN) | [@jnMetaCode](https://github.com/jnMetaCode) | [agency-agents-zh](https://github.com/jnMetaCode/agency-agents-zh) | 141 translated agents + 46 China-market originals |
| 🇨🇳 (zh-CN) | [@dsclca12](https://github.com/dsclca12) | [agent-teams](https://github.com/dsclca12/agent-teams) | Independent translation with Bilibili, WeChat, Xiaohongshu localization |
...
| 🇯🇵 (ja-JP) | [@sscodeai](https://github.com/sscodeai) | [agency-agents-ja](https://github.com/sscodeai/agency-agents-ja) | 281 Japan-localized agents + 97 Japan-market originals + 27 workflows |
